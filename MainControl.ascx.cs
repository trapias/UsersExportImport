...
namespace forDNN.Modules.UsersExportImport
{
	partial class MainControl : PortalModuleBase, IActionable
    {
        ...
        //@trapias: load separator char from settings
		...
		private void ExtraPageLoad()
		{
            ...
            //@trapias: load separator char from settings
            if ((string)Settings["Separator"]!=null &&  (string)Settings["Separator"]!="")
            {
                LoggerSource.Instance.GetLogger(this.GetType()).Debug("UsersExportImport using Separator: " + (string)Settings["Separator"]);
                Separator = (string)Settings["Separator"];
                separatorChar = Convert.ToChar( (string)Settings["Separator"] );
            }
		...
		{
            //string Separator = ",";
           //LoggerSource.Instance.GetLogger(this.GetType()).Debug("CSV2Table separator=" + Separator);
            ...
            foreach (string Header in lstRows[0].Split(new char[] { separatorChar }))
			{
...
");
			...
			switch (ddlExportFileType.SelectedValue)
			{
				...
				case "1"://XML - FOR XML RAW
					...
					break;
				case "2"://CSV
                    strResult = CommonController.ToCSV(dt, true, Separator);
